...
booksController.issueBook = async (req, res) => {
  try {
    ...
    if (issuedBooksCount >= 4) {
      return res
        .status(400)
        .json({ message: "You can issue a maximum of 4 books at a time." });
    }
  ...
  try {
    ...
    if (currentCount >= 4) {
      return res.status(400).json({
        error: true,
        message: "You cannot request or issue more than 4 books at a time.",
      });
    }
...
booksController.searchBooks = async (req, res) => {
  try {
    ...
    const books = await BookModel.find(filter)
      .populate("addedBy", "name email role")
      .sort({ createdAt: -1 });
    res.status(200).json({ error: false, books, total: books.length });
  } catch (error) {
    res.status(500).json({ error: true, message: "Internal Server Error" });
  }
};

booksController.getCategories = async (req, res) => {
  try {
    const categories = await BookModel.distinct("category", {
      category: { $ne: null, $ne: "" },
    });
    res.status(200).json({ error: false, categories: categories.sort() });
  } catch (error) {
    res.status(500).json({ error: true, message: "Internal Server Error" });
  }
};
